...
import SocialButtons from "../../../components/common/SocialButtons/SocialButtons";
import PasswordInput from "../../../components/common/PasswordInput/PasswordInput";
import { useAuth } from "../../../context/AuthContext"; // استيراد السياق
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "/api/auth/login", // استخدام المسار النسبي بفضل proxy
        {
          email: formData.email,
          password: formData.password,
        }
      ...
      if (response.data.success) {
        ...
        // استخدم دالة login من السياق لتخزين التوكن والمستخدم
